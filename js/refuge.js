// Mensajes para el termómetro de mariposas (directos, cortos, potentes)
...
// Clóset: mensaje nocturno
document.getElementById('closet-btn')?.addEventListener('click', () => {
    const msgDiv = document.getElementById('closet-message');
    msgDiv.innerHTML = `“Los niños ya aprendieron sus figuras hoy, ya descansan. Ahora este espacio es nuestro. El frío de la montaña, el café y mis brazos te esperan para cuando estrenes esa pijama corta frente a la chimenea. Descansa, mi niña hermosa, gracias por hacer mejores tus días.” ❄️🔥`;
});

// ALIVIO SÓNICO con Web Audio API (ruido blanco + viento + lluvia)
let audioCtx = null;
let sourceNode = null;
let isPlaying = false;
...
function createWindAndRain() {
    ...
    // Filtro para simular viento (pasa bajos)
    ...
    noiseSource.start();
    // Añadir componente de lluvia (ruido rosado)
    ...
    rainSource.start();
    return { noiseSource, rainSource, gain, rainGain };
}

let activeSources = null;
document.getElementById('sonic-relief')?.addEventListener('click', () => {
    if (isPlaying) {
        // Si ya suena, lo detenemos y reiniciamos (para evitar duplicados)
        if (activeSources) {
            activeSources.noiseSource.stop();
            activeSources.rainSource.stop();
        }
        if (audioCtx) audioCtx.close();
        isPlaying = false;
    }
...
// Oscurecer pantalla para aliviar migraña
